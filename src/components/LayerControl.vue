  ...
  <div class="layer-control">
    <v-container fluid >
      <v-layout column wrap >
        ...
        <v-flex xs12 v-if="selectHazards">
          Return period
          <v-divider></v-divider>
          <v-radio-group v-model="returnPeriod" v-if="selectHazards.text === 'Coastal Flood'">
            <v-layout row>
              <v-radio v-for="n in [25, 100, 500]" :key="n" :label="` ${n}`" :value="n"></v-radio>
            </v-layout>
          </v-radio-group>
          <v-radio-group v-model="returnPeriod" v-if="selectHazards.text === 'EarthQuake'">
            <v-layout row>
        ...
        <v-flex v-if="exposureLayer">
          School Exposure
          <v-divider></v-divider>
          <v-layout row wrap >
            <v-flex xs3>
              <v-switch v-model="exposureLayer.active" hide-details></v-switch>
            </v-flex>
            <v-flex xs7>{{exposureLayer.name}}</v-flex>
    ...
    <v-list pt-0 v-if="hazardLayer">
      Hazards
      <v-divider></v-divider>
      <v-list-tile>
        <v-list-tile-action>
          <v-switch v-model="hazardLayer.active" v-if="hazardLayer"></v-switch>
        </v-list-tile-action>
        <v-list-tile-title>
          <v-select
            v-model="selectHazards"
        ...
        <v-list-tile-action>
          <v-icon >{{hazardLayer.icon}}</v-icon>
        </v-list-tile-action>
      </v-list-tile>
